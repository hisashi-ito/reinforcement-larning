  ...
  def _step(self, action):
    ...
    if self._state == 0:#電源OFFの状態
      if action == 0:#電源ボタンを押す
        self._state = 1#電源ON
        GPIO.output(15, 1)#電源LED ON
      else:#行動ボタンを押す
        self._state = 0#電源OFF
        GPIO.output(15, 0)#電源LED OFF
    else:#電源ONの状態
      if action == 0:
        self._state = 0
        GPIO.output(15, 0)#電源LED OFF
    ...
    img = cv2.resize(gray[yp-d:yp + d, xp-d:xp + d],(SIZE, SIZE))#画像の中心を切り抜いて16×16の画像に変換
    img = img/256.0#0～1に正規化
    img_state = img.reshape(SIZE, SIZE, 1) # 3次元行列に変換（16×16×1，縦×横×チャンネル数）
