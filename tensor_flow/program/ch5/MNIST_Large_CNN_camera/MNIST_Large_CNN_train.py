'''
MNISTを用いた数字分類のためのCNN学習プログラム（28x28画像使用，画像の正規化）
...
def main():
  # データの作成
  (train_data, train_label), (valid_data, valid_label) = tf.keras.datasets.mnist.load_data()
  train_data, valid_data = train_data / 255.0, valid_data / 255.0
  train_data = train_data.reshape((len(train_data), 28, 28, 1))
  valid_data = valid_data.reshape((len(valid_data), 28, 28, 1))
  # ニューラルネットワークの登録
